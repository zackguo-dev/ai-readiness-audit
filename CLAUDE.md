...
- **毎回自動で** `results/{ドメイン}/{ISO8601タイムスタンプ}.json` に機械可読な
  診断結果を保存する(url/final_url/fetched_at/総合スコア/check別score・findings・
  recommendations)。履歴比較・グラフ化・差分表示はv1スコープ外(BACKLOG参照)
